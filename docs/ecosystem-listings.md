...
1. File an issue first (their CONTRIBUTING asks for it on significant work):
   ...
   >
   > Agent402 is an open-source x402 + MCP server with 1,346 pay-per-call web
...
2. Fill the form:
   ...
   - **Short description (one line, ~150 chars):**
     ...
     ```
     1,346 pay-per-call web tools + 42 skill packs for AI agents over x402 (USDC on Base + 3 more chains, or USDG on Robinhood Chain) — or free via proof-of-work. Browser, search, OCR, finance, EDGAR, durable memory.
     ```
   ...
   - **Long description / why (if asked):**
     ...
     Also publishes 42 curated multi-tool skill packs as MCP prompts.
     ```
...
2. Fill:
   ...
   - **One-liner:**
     ...
     ```
     1,346 pay-per-call web tools + 42 skill packs for AI agents over x402 — USDC on Solana (and Base/Polygon/Arbitrum) or free via proof-of-work. Open-source, self-hostable, MCP-native.
     ```
